...
# Todas las filas, los 4 primeros valores
inputs = dataset[:,1:4];
...
# Lo mete en un array de 2 dimensiones de floats
inputs = [Float32(x) for x in inputs];
# convierte cada elemento a su punto flotante para poder operar con los numeros. 1: filas. 2: columnas. de tipo flotante
println("Tamaño de la matriz de entradas: ", size(inputs,1), "x", size(inputs,2), " de tipo ", typeof(inputs));

# Preparamos las salidas deseadas codificándolas puesto que son categóricas hace un array de 1 columna por todas las filas que haya en data set y pilla los resultados.
targets = dataset[:,5];
